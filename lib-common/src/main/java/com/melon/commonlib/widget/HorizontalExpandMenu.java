...
public class HorizontalExpandMenu extends RelativeLayout {
    ...
    private void init() {
        ...
        defaultWidth = DisplayUtil.dip2px(200);
        defaultHeight = DisplayUtil.dip2px(40);
        ...
        menuCornerRadius = typedArray.getDimension(R.styleable.HorizontalExpandMenu_corner_radius, DisplayUtil.dip2px(20));

        buttonStyle = typedArray.getInteger(R.styleable.HorizontalExpandMenu_button_style, Right);
        buttonIconDegrees = 90;
        buttonIconSize = typedArray.getDimension(R.styleable.HorizontalExpandMenu_button_icon_size, DisplayUtil.dip2px(8));
        ...
        isExpand = typedArray.getBoolean(R.styleable.HorizontalExpandMenu_expand, false);
        typedArray.recycle();
                ...
                if (backPathWidth == maxBackPathWidth || backPathWidth == 0) {//动画结束时按钮才生效
                    switch (buttonStyle) {
                        case Right:
                            ...
                            break;
                        case Left:
                            if (x >= leftButtonLeft && x <= leftButtonRight) {
                                expandMenu(expandAnimTime);
                            }
                            break;
                    }
...
//        canvas.restore();
    ...
    private void expandMenu(int time) {
        ...
        if (expandListener != null)
            expandListener.expandMenu(isExpand);
    ...
    public interface ExpandListener {
        void expandMenu(boolean isExpand);
    }
    ...
    public void setOnExpandListener(ExpandListener listener) {
